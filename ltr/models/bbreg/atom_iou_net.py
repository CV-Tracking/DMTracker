...
class AtomIoUNet(nn.Module):
    ...
    def forward(self, feat1, feat2, bb1, proposals2): #torch.Size([3, 1, 4]) torch.Size([3, 1, 8, 4])
        ...
        feat1 = [f[0,...] if f.dim()==5 else f.reshape(-1, num_sequences, *f.shape[-3:])[0,...] for f in feat1] #[torch.Size([1, 512, 36, 36])]
        bb1 = bb1[0,...] #torch.Size([1, 4])
    ...
    def get_modulation(self, feat, bb):
        ...
        roi4r = self.prroi_pool4r(c4_r, roi1) #c4_r:torch.Size([1, 256, 18, 18])  roi1:torch.Size([1, 5]) 

        fc3_r = self.fc3_1r(roi3r) #roi3r: torch.Size([1, 128, 3, 3])
