...
class FeatureFusionNetwork(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_featurefusion_layers=4,
        ...
        featurefusion_En_layer = En(d_model, nhead, dim_feedforward, dropout, activation)
        featurefusion_De_layer = De(d_model, nhead, dim_feedforward, dropout, activation)
        self.fusion = Fusion(featurefusion_En_layer, featurefusion_De_layer, num_featurefusion_layers)
    ...
    def forward(self, src_temp, mask_temp, src_search, mask_search, pos_temp, pos_search):
        ...
        out = self.fusion(src1=src_temp, src2=src_search,
        ...
        hw, b, c = out.shape
        h = w = int(hw**0.5)
        out = out.view(h, w, b, c).permute(2, 3, 0, 1)
        return out
...
class Fusion(nn.Module):
    ...
    def __init__(self, featurefusion_En_layer, featurefusion_De_layer, num_layers):
        super().__init__()
        self.En_layers = _get_clones(featurefusion_En_layer, num_layers)
        self.De_layers = _get_clones(featurefusion_De_layer, num_layers)
    ...
    def forward(self, src1, src2,
        ...
        #src1 rgb q , src2 depth kv
        for layer in self.En_layers:
            info = layer(src1, src2, src1_mask=src1_mask,
                                     src2_mask=src2_mask,
        ...
        for layer in self.De_layers:
            output1 = layer(src2, info, src1_mask=src2_mask,
                                     src2_mask=src1_mask,
                                     src1_key_padding_mask=src2_key_padding_mask,
                                     src2_key_padding_mask=src1_key_padding_mask,
...
class En(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        #self.filter = nn.Conv2d(in_channels=d_model, out_channels=d_model, kernel_size=1,stride=1, padding=0)
    ...
    def forward_post(self, src1, src2,
        ...
        out = self.filter(x.view(b,c,int(s**0.5),int(s**0.5)), y.contiguous().view(b,c,int(s**0.5),int(s**0.5))).view(b,c,-1).permute(2, 0, 1)
