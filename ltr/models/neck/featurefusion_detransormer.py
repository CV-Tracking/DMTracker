...
class FeatureFusionNetwork(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_featurefusion_layers=4,
        ...
        featurefusion_CA_layer = CA(d_model, nhead, dim_feedforward, dropout, activation)
        self.fusion = Fusion(featurefusion_CA_layer, num_featurefusion_layers)
...
class Fusion(nn.Module):
    ...
    def __init__(self, featurefusion_CA_layer, num_layers):
        super().__init__()
        
        self.CA_layers = _get_clones(featurefusion_CA_layer, num_layers)
    ...
    def forward(self, src1, src2,
        ...
        for layer in self.CA_layers:
            src1 = layer(src1, src2, src1_mask=src1_mask,
                                     src2_mask=src2_mask,
...
class CA(nn.Module):
    ...
    def forward_post(self, src1, src2,
        ...
        q1 = k1 = self.with_pos_embed(src1, pos_src1)
        src11 = self.self_attn1(q1, k1, value=src1, attn_mask=src1_mask,
                               key_padding_mask=src1_key_padding_mask)[0]

        src1 = src1 + self.dropout1(src11)
        src1 = self.norm1(src1)
        src12 = self.activation1(self.linear1(src1))
        src1 = src1 + self.dropout2(src12)
        ...
        src12 = self.linear3(self.dropout4(self.activation2(self.linear2(src1))))
        src1 = src1 + self.dropout5(src12)
