...
class DiMP(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        # return out, score_map.cpu().data.numpy(), im_patches[0].cpu().data.numpy()
        return out
    ...
    def extract_backbone_features(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        ...
        with torch.no_grad():
            backbone_feat = self.net.extract_backbone_DETT(im_patches, model='test') #{'layer2': tensor([[[[0.0023, 0...='cuda:0'), torch.Size([1, 512, 36, 36]) 'layer3': tensor([[[[0.7306, 0...='cuda:0')}
    ...
    def generate_init_samples(self, im: torch.Tensor) -> TensorList:
        ...
        with torch.no_grad():
            init_backbone_feat = self.net.extract_backbone_DETT(im_patches, model='train')
